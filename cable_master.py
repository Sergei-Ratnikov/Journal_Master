...
class CableParserGUI:
    ...
    def __init__(self):
        ...
        if getattr(sys, 'frozen', False):
            # Если программа запущена как .exe (упакована PyInstaller)
            program_dir = os.path.dirname(sys.executable)
        else:
            # Если запущена как .py скрипт
            program_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Переменные для вкладки 1 (Парсер)
        self.journals_dir = tk.StringVar(value=program_dir)
        self.output_dir = tk.StringVar(value=program_dir)

        
        # Переменные для вкладки 2 (Поиск ответных частей)
        self.matcher_db_path = tk.StringVar(value=program_dir)
    ...
    def open_result_folder(self):
        """Открывает папку с результатом (для вкладки Парсер)"""
    ...
    def setup_matcher_tab(self):
        ...
        self.matcher_open_folder_btn = tk.Button(
            button_frame,
            text="Открыть папку с результатом",
            command=self.open_matcher_result_folder,
            bg="#3498db",
            fg="white",
            font=("Arial", 9),
            padx=10,
        ...
        self.matcher_open_folder_btn.pack(side="left")
    ...
    def open_matcher_result_folder(self):
        """Открывает папку с результатом (для вкладки Поиск ответных частей)"""
        db_path = Path(self.matcher_db_path.get())
        reports_dir = db_path.parent / "Отчёты"
        if reports_dir.exists():
            self.open_folder_path(str(reports_dir))
        else:
            self.update_status("Папка 'Отчёты' ещё не создана", is_error=True)
    ...
    def _on_parser_success(self, start_time):
        ...
        self.open_folder_btn.config(state="normal")  # ← активируем кнопку
    ...
    def _on_parser_error(self, error_msg):
        self.is_running = False
        self.run_btn.config(state="normal", bg="#27ae60")
        self.open_folder_btn.config(state="disabled")  # ← деактивируем
    ...
    def _on_matcher_success(self, start_time, output_path):
        ...
        self.matcher_open_folder_btn.config(state="normal")  # ← активируем кнопку
        self.update_status(f"✅ Обработка завершена! Время: {duration:.2f} сек")
    ...
    def _on_matcher_error(self, error_msg):
        self.is_running = False
        self.matcher_run_btn.config(state="normal", bg="#8e44ad")
        self.matcher_open_folder_btn.config(state="disabled")  # ← деактивируем
        self.update_status(f"Ошибка: {error_msg}", is_error=True)
        messagebox.showerror("Ошибка", f"При обработке произошла ошибка:\n\n{error_msg}")
