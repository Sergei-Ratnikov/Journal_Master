...
class CableParserGUI:
    ...
    def setup_matcher_tab(self):
        ...
        output_frame = tk.LabelFrame(main_frame, text="Сохранение результата", 
                                     padx=10, pady=10, font=("Arial", 10, "bold"), bg="#f0f0f0")
        output_frame.pack(fill="x", pady=(0, 15))
        
        tk.Entry(output_frame, textvariable=self.matcher_output_path, 
                font=("Arial", 10), state="readonly", bg="white").pack(side="left", fill="x", expand=True, padx=(0, 10))
        tk.Button(output_frame, text="Выбрать файл", command=self.select_matcher_output,
                 bg="#3498db", fg="white", font=("Arial", 9), padx=10, cursor="hand2").pack(side="right")
    ...
    def paste_from_clipboard(self):
        """Вставляет текст из буфера обмена в поле ККС журнала"""
        try:
            clipboard_text = self.root.clipboard_get()
            if clipboard_text:
                self.matcher_journal_kks.set(clipboard_text.strip())
                self.update_status("✅ ККС вставлен из буфера обмена")
        except:
            self.update_status("❌ Не удалось получить данные из буфера обмена", is_error=True)
    ...
    def _on_parser_success(self, start_time):
        ...
        if self.open_folder.get():
            self.root.after(500, lambda: self.open_folder(self.output_dir.get()))
    ...
    def run_matcher(self):
        ...
        if not self.matcher_output_path.get():
    ...
    def _process_matcher(self):
        ...
        try:
            process_journal(
                ...
                self.matcher_journal_kks.get().strip(),
                self.matcher_output_path.get()
            )
            self.root.after(0, self._on_matcher_success, start_time)
        except Exception as e:
            self.root.after(0, self._on_matcher_error, str(e))
    
    def _on_matcher_success(self, start_time):
        duration = time.time() - start_time
        self.is_running = False
        ...
        if self.matcher_open_folder.get():
            output_dir = os.path.dirname(self.matcher_output_path.get())
            self.root.after(500, lambda: self.open_folder(output_dir))
        
        messagebox.showinfo("Готово!", 
            f"✅ Поиск завершён!\n\n"
            f"📁 База: {self.matcher_db_path.get()}\n"
            f"📌 Журнал: {self.matcher_journal_kks.get()}\n"
            f"💾 Результат: {self.matcher_output_path.get()}\n"
            f"⏱️ Время: {duration:.2f} сек")
