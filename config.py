...
# ----- Регулярное выражение для имени файла журнала -----
regular_filename = re.compile(
    r'^(?P<kks>AKU\.\d{2,4}\.\d{2}[A-Z]{3}\.[A-Z0-9.]+-[A-Z0-9]+)'  # ККС журнала
    r'(?:_(?P<source>ВКРД|ВКИИ|СРД|СИИ))?'                          # шифр источника (опционально)
    r'(?:_(?P<date>\d{2}\.\d{2}\.\d{4}))?'                          # дата (опционально)
    r'(?:\.docx?)?$'                                                # расширение файла
)
...
HEADERS = [
    ...
    [23, 'Номер ревизии', 6],
    [24, 'Дата ревизии', 10],
    [25, 'Источник информации', 8],
    [26, 'Класс безопасности', 7],
    [27, 'Статус прокладки', 10],
    [28, 'Статус объединения', 10],
    [29, 'Raw', 25],
    [30, 'min длина', 10],
    [31, 'Требования к объединению', 20],
    [32, 'Ответная часть (из КЖ)', 40],
    [33, 'Ответный журнал', 20],  
    [34, 'Ответный кабель', 20],      
    [35, 'Примечание', 40]
]